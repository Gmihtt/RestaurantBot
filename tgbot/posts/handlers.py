...
from tgbot.filters import find_callback_text, check_state
...
def add_post_handlers(bot: AsyncTeleBot):
    bot.register_callback_query_handler(add_post.post_name_message,
                                        func=lambda c: find_callback_text(c, "add_post"),
                                        pass_bot=True)
    ...
    bot.register_callback_query_handler(add_post.post_file_message,
                                        func=lambda c: find_callback_text(c, "add_new_photo_post"),
                                        pass_bot=True)
